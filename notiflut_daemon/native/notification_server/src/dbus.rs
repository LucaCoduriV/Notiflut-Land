use std::{collections::VecDeque, error::Error, sync::mpsc::Sender, time::Duration};
...
impl dbus_definition::OrgFreedesktopNotifications for DbusNotification {
    ...
    fn close_notification(&mut self, id: u32) -> Result<(), dbus::MethodErr> {
        if let Err(_) = self
            .sender
            .send(ChannelMessage::DbusEvent(DbusEvent::CloseNotification(id)))
        {
    ...
    fn notify(
        ...
        if let Err(_) = self
            .sender
            .send(ChannelMessage::DbusEvent(DbusEvent::NewNotification(
                notification,
            )))
        {
            return Err(dbus::MethodErr::failed(
    ...
    pub fn register_notification_handler(
        ...
        let token = crossroad.register(NOTIFICATION_INTERFACE, |builder| {
            ...
            builder.method("OpenNC", (), ("reply",), move |_, _, ()| {
                sender_clonded
                    .send(ChannelMessage::DbusEvent(DbusEvent::ShowNotificationCenter))
            ...
            builder.method("CloseNC", (), ("reply",), move |_, _, ()| {
                sender_clonded
                    .send(ChannelMessage::DbusEvent(DbusEvent::HideNotificationCenter))
                    .map_err(|e| MethodErr::failed(&e))?;
